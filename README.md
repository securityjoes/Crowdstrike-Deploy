...
2. Run the script, and upload the 6 parts to Defender library and then upload them from Defender library in to the host. (the default library upload path on Defender for Linux is "/var/opt/microsoft/mdatp/response" for Windows is "C:\ProgramData\Microsoft\Windows Defender Advanced Threat Protection\Downloads").
